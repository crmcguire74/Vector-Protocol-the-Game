...
export class Input {
  ...
  bindXR(renderer, onSqueezePause) {
    for (let i = 0; i < 2; i++) {
      ...
      obj.addEventListener("squeezestart", () => {
        ...
        this.push("gripSqueeze", { hand: c.hand });
      });
  ...
  pollXR(dt, mode, xrCamera) {
    ...
    for (const c of this.ctrls) {
      ...
      if (gp && gp.axes.length >= 4) {
        ...
        if (mode === "cycle") {
          // Flicking either stick past the gate fires exactly one 90-degree turn.
          if (x < -0.6 && c.stickPrevX >= -0.6) this.push("turnL");
          if (x > 0.6 && c.stickPrevX <= 0.6) this.push("turnR");
          if (c.hand === "right" && Math.abs(y) > 0.28) this.xrDashAdjust = -y;
        ...
        c.stickPrevX = x;
        if (mode === "cycle") {
          // Throttle asks for BOTH index triggers; grips (thumb-side squeeze) brake.
          const trigger = (gp.buttons[0] && gp.buttons[0].value) || 0;
          if (trigger > 0.35) {
            if (c.hand === "left") this.xrTrigL = true;
            else this.xrTrigR = true;
          }
          if (c.squeezeHeld) this.xrBrake = true;
        }
      }
      // face buttons: A/X (4) = hop / boost burst, B/Y (5) = pause
      if (gp && gp.buttons.length > 4) {
        const face = !!(gp.buttons[4] && gp.buttons[4].pressed);
        if (face && !c.facePrev) this.push(mode === "cycle" ? "boostBurst" : "hop", { hand: c.hand });
        c.facePrev = face;
  ...
  // sustained accelerator: both XR index triggers, or W / pad A / touch hold
  throttleHeld(inXR) {
    return inXR ? !!(this.xrTrigL && this.xrTrigR)
      : (this.keys.has("KeyW") || this.keys.has("ArrowUp") || this.padBoost || this.touch.boost);
  }
