# Vector Protocol - numeric thresholds (fixed before code)
...
## v7 revisions (2026-07-20)
- Cycle VR controls rebuilt to the discrete spec: stick-X edge past 0.6 on either stick
  fires exactly one 90-degree turn; body lean fires one turn at 0.22 m lateral headset
  offset against an adaptive neutral center (damp 0.25/s armed, 1.6/s disarmed,
  re-arm after 0.3 s inside 0.10 m). Straightening up or the post-turn frame rotation
  never fires a phantom turn. No analog steer, no camera roll (XR roll locked to 0).
- Speed model: BOTH index triggers held = throttle x1.35 (W on desktop / touch hold);
  A/X (Shift) = boost surge x1.85 for 1.15 s off the meter; either grip squeeze
  (S / touch) = brake x0.55. Emergency stop removed.
- Disc VR: squeezing a grip while the player disc is in flight recalls it (R on web).
- AR: portals are lethal. Stepping on / brushing / being knocked through any breach
  triggers a 1.4 s gridFall derez (vignette close + 70-shard burst) and the round.
  Enemy programs die instantly through portals (spin-down + burst death anim kept).
- Cycle model v7 (Legacy silhouette): dark torus tires with hot inner rim rings and
  additive energy-disc hubs, longer hugging fenders (arcs 0.9/1.02 pi), stretched
  low fuselage, flatter glass canopy, embedded flank light tubes, rear spine strip
  (enemy variant), subtler beacon. Per-cycle turn bank anim (0.45 rad, damp 5.5).
- Trails: second instanced pass adds a white-hot 0.06 m top edge per ribbon.
- Environment: Legacy grid floor (roughness 0.14), 12 m energy walls, instanced
  22-tower skyline ring (2 draws), 4-tier mega-tower with apex beam at z -330,
  additive horizon band; scene fog widened to 70-680. Disc court gets a 16-tower
  ring + horizon at its own center. Measured: cycle mode ~80 calls / 116k tri
  desktop at 60 fps; disc ~75 calls.

## v6 revisions (2026-07-12)
- Disc trail: 12 shared dynamic points as one additive line + one additive Points draw;
  planar non-billboard aura keeps the thrown profile horizontal in stereo XR.
- AR footprint: bounded-floor polygon preferred, detected horizontal plane fallback,
  manual 7 x 8 m setup fallback. Breaches live 7 s; opening radius 0.62 m; opening-loss
  cooldown 1.8 s; a direct hit plus a knock through an opening costs two integrity pips.
- Cycle LIGHTFIELD half-extent 82 m; speeds 14 / 18 / 22 m/s; countdown 5 s; AI grace
  8 s. Trigger boost burst 0.9 s. Emergency stop duration 1.05 s, three uses/round.
  Dashboard offset range -0.28..0.38 m. Body lean reaches full steer at 0.30 m lateral
  headset displacement; analog turn charge accelerates with lean magnitude.
