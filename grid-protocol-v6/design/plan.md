...
2. CYCLE RUN - first-person light cycle survival vs 3 AI in a compact 92 m square
   LIGHTFIELD. Rivals start in a visible formation, then begin hunting after 5.5 s.
...
- VR: trigger hold+swing+release = throw (controller velocity), off-hand grip = guard,
  left stick = locomotion nudge (disc) / snap turns (cycle), trigger = boost, grip = brake.
...
no artificial player push in disc mode (hits shatter floor instead), AR cycle mode is a
floor-anchored holo-table diorama.
