...
export class DiscMode {
  ...
  build(g) {
    ...
    // distant void architecture
    this.monoliths = new THREE.Group();
    for (let i = 0; i < 3; i++) {
      const a = (i / 3) * Math.PI * 2 + 0.4;
      const m = kit.makeMonolith(8 + Math.random() * 8, 34 + Math.random() * 30, 5 + Math.random() * 4);
      m.position.set(Math.cos(a) * (95 + Math.random() * 40), 0, -10 + Math.sin(a) * (95 + Math.random() * 40));
      m.rotation.y = Math.random() * Math.PI;
      this.monoliths.add(m);
    }
  ...
  losePlayerToOpening(g) {
    this.playerPips--;
    this.openingCooldown = 1.8;
    g.banner([STR.openingFall], 1.2);
    g.audio.hurt(); g.flash(0.5);
    if (this.playerPips <= 0) this.roundOver(g, false, true);
  ...
  loseEnemyToOpening(g) {
    this.enemy.pips--;
    this.openingCooldown = 1.8;
    g.banner([STR.enemyOpeningFall], 1.2);
    g.audio.sfx("sfx_shatter", { pos: this.enemy.pos });
    this.enemy.pos.set(this.roam.cx, 0, this.roam.cz);
  ...
  hitEnemy(g) {
    this.enemy.pips--;
    const knockedThrough = this.ar && this.nearOpening(this.enemy.pos.x, this.enemy.pos.z);
    if (knockedThrough) {
      this.enemy.pips--;
      this.openingCooldown = 1.8;
      g.banner([STR.enemyOpeningFall], 1.2);
  ...
  hitPlayer(g) {
    ...
    const knockedThrough = this.ar && this.nearOpening(head.x, head.z);
    if (knockedThrough) {
      this.playerPips--;
  ...
  update(g, dt, events) {
    ...
    if (inPlay) {
      ...
      if ((floorOpening || wallOpening) && !this.playerOpeningLatched && (this.openingCooldown || 0) <= 0) {
        this.playerOpeningLatched = true;
        this.losePlayerToOpening(g);
      } else if (!floorOpening && !wallOpening) {
        this.playerOpeningLatched = false;
      }
      if (this.grounded) {
