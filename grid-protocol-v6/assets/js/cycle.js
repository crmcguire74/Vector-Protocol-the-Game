...
const BOOST_MULT = 1.6, BOOST_DRAIN = 1 / 1.2, BOOST_REGEN = 1 / 3;
...
class Cycle {
  ...
  spawn(x, z, dirIdx) {
    ...
    for (let i = 0; i < MAX_SEG; i++) this.trail.setMatrixAt(i, zero.matrix);
...
export class CycleMode {
  ...
  build(g) {
    ...
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(ARENA * 2, ARENA * 2),
      new THREE.MeshBasicMaterial({ map: floorTex, transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false }));
    floor.rotation.x = -Math.PI / 2;
    this.group.add(base, floor);
    // distant architecture beyond the walls
    for (let i = 0; i < 4; i++) {
      const m = kit.makeMonolith(10 + Math.random() * 10, 40 + Math.random() * 35, 6 + Math.random() * 5);
      const a = (i / 4) * Math.PI * 2 + 0.7;
      m.position.set(Math.cos(a) * (150 + Math.random() * 60), 0, Math.sin(a) * (150 + Math.random() * 60));
    ...
    const wallMat = new THREE.MeshBasicMaterial({ map: wallTex, color: COL.cyanHi, transparent: true, opacity: 0.72, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
    for (let i = 0; i < 4; i++) {
      const w = new THREE.Mesh(new THREE.PlaneGeometry(ARENA * 2, 8), wallMat);
      w.position.y = 4;
  ...
  update(g, dt, events) {
    ...
    for (const e of events) {
      ...
      else if (e.type === "emergencyStop" && this.stopsLeft > 0 && this.stopCooldown <= 0) {
        this.stopsLeft--;
        this.stopT = 1.05;
        this.stopCooldown = 0.3;
        g.banner([`${STR.emergencyStop}: ${this.stopsLeft} ${STR.stopsLeft}`], 1.0);
        g.audio.tone(54, 0.18, "square", 0.5);
        g.hapticDisc(0.8, 100); g.hapticGuard(0.8, 100);
      }
    }

    // ---- analog steering: thumbstick or physical handlebar lean. More lean
    // fills the turn charge faster, producing a sharper series of Grid turns.
    let steer = inXR ? g.input.cycleSteer() : 0;
    if (inXR && !this.holo && this.state === "run") {
      // Headset translation supplies true body-lean steering. The calibrated
      // center is captured at each round start; farther lean turns sooner.
      g.headWorld(_v1);
      g.rig.worldToLocal(_v1);
      if (this.leanCenterX == null) this.leanCenterX = _v1.x;
      const bodyLean = clamp((_v1.x - this.leanCenterX) / 0.3, -1, 1);
      if (Math.abs(bodyLean) > 0.08) steer = clamp(steer + bodyLean, -1, 1);
      if (g.input.bothGripsHeld()) {
        const ly = g.input.handY("left"), ry = g.input.handY("right");
        if (ly != null && ry != null) {
          const lean = clamp((ry - ly) / 0.3, -1, 1);
          steer = clamp(steer - lean * 0.45, -1, 1);
        }
      }
      this.steerCooldown = Math.max(0, this.steerCooldown - dt);
      this.steerVisual = damp(this.steerVisual, steer, 9, dt);
      this.steerCharge += steer * dt * (0.65 + Math.abs(steer) * 2.4);
      if (this.steerCooldown <= 0 && Math.abs(this.steerCharge) >= 0.5) {
        this.doTurn(g, player, this.steerCharge < 0);
        this.steerCharge = 0;
        this.steerCooldown = 0.22;
      }
    }

    // ---- accelerator / boost / brake
    const boostHeld = g.input.boostHeld(inXR);
    if (boostHeld && !this.boostWas) this.boostBurstT = 0.9;
    this.boostBurstT = Math.max(0, this.boostBurstT - dt);
    player.boosting = (boostHeld || this.boostBurstT > 0) && player.boost > 0.05;
    if (player.boosting) {
      player.boost = Math.max(0, player.boost - BOOST_DRAIN * dt);
      if (!this.boostWas) g.audio.boost();
    } else {
      player.boost = Math.min(1, player.boost + BOOST_REGEN * dt);
    }
    this.boostWas = player.boosting;
    this.stopT = Math.max(0, this.stopT - dt);
    this.stopCooldown = Math.max(0, this.stopCooldown - dt);
    const braking = g.input.brakeHeld(inXR);
    this.playerMult = this.stopT > 0 ? 0 : player.boosting ? BOOST_MULT : braking ? BRAKE_MULT : 1;
    ...
    if ((this.time * 10 | 0) !== this.hudTick) {
      ...
      g.hud.set({
        ...
        pips: null, epips: null, disc: `${STR.emergencyStops}: ${this.stopsLeft}`,
      });
    }
  ...
  integrate(g, dt, endgame) {
    ...
    for (const c of cycles) {
      if (!c.alive) { c.fade = Math.max(0, c.fade - dt); c.trailMat.opacity = Math.min(1, c.fade); continue; }
      let spd = this.speed;
  ...
  frame(g, rdt, alpha, time) {
    ...
    this.cycles[0].mesh.rotation.z = damp(this.cycles[0].mesh.rotation.z, -this.steerVisual * 0.34, 10, rdt);
    if (g.inXR) g.camera.rotation.z = damp(g.camera.rotation.z, -this.steerVisual * 0.055, 10, rdt);
    if (pc.canopy) {
